...
def process_single_file(chm_path, dtm_path, dsm_path, building_geometries, output_base_folder, nodata_value=-9999,
    ...
    """
    Process a single CHM file, generating and saving the final DSM, CHM, and additional filled DSM and DTM.
    """
    # Extract tile and file info
    ...
    # Overlapping bounding box and cropping
    ...
    # Fill missing values in DSM and DTM
    filled_dtm, filled_dtm_transform = fill_raster(dtm_cropped, nodata_value, dtm_transform, speed_up=speed_up)
    filled_dsm, filled_dsm_transform = fill_raster(dsm_cropped, nodata_value, dsm_transform, speed_up=False)

    # Save the filled DTM
    filled_dtm_filename = f"Filled_DTM_{tile}_{file_number}.tif"
    output_filled_dtm_path = os.path.join(output_folder, filled_dtm_filename)
    write_output(rasterio.open(dtm_path), filled_dtm, filled_dtm_transform, output_filled_dtm_path, True)

    # Save the filled DSM
    filled_dsm_filename = f"Filled_DSM_{tile}_{file_number}.tif"
    output_filled_dsm_path = os.path.join(output_folder, filled_dsm_filename)
    write_output(rasterio.open(dsm_path), filled_dsm, filled_dsm_transform, output_filled_dsm_path, True)

    # Generate and save CHM
    ...
    # Replace buildings in filled DTM with values from DSM
    final_dsm_with_buildings = replace_buildings(filled_dtm, filled_dsm, building_geometries, filled_dsm_transform)
    final_dsm_with_buildings_filename = f"DSM_Building_{tile}_{file_number}.tif"
    output_final_dsm_with_buildings_path = os.path.join(output_folder, final_dsm_with_buildings_filename)
    write_output(rasterio.open(dtm_path), final_dsm_with_buildings, filled_dsm_transform,
                 output_final_dsm_with_buildings_path, True)

    print(f"Processed {chm_filename}, outputs saved to {output_folder}")
...
#     chm_folder = "D:\Geomatics\__report files\chm"
...
#     output_base_folder = "D:\Geomatics\__report files"
#
...
tif_file_path = "D:\Geomatics\__report files/25DN2\DSM_Building_25DN2_10.tif"
geopackage_path = "C:\Geomatics\synthesis\_newfigures/buildingsfixed/ams.gpkg"
output_tif_path = "D:\Geomatics\__report files/25DN2\DSM_Building.tif"

# Step 1: Load the GeoPackage and read geometries
gdf = gpd.read_file(geopackage_path)
geometries = [feature["geometry"] for feature in gdf.__geo_interface__["features"]]

# Step 2: Open the TIFF file and apply the mask
with rasterio.open(tif_file_path) as src:
    # Mask the raster with the geometries from the GeoPackage
    out_image, out_transform = mask(src, geometries, crop=False, nodata=src.nodata)

    # Update metadata to set the new nodata area
...
# Step 3: Write the result to a new TIFF file
